...
def upgrade():
    ...
    # Robustness: Drop tables if they exist before creating them
    # Following the downgrade order to respect foreign keys

    # Disable foreign key checks to allow dropping referenced tables
    op.execute("SET FOREIGN_KEY_CHECKS = 0;")

    op.drop_table('waitlists', if_exists=True)
    op.drop_table('votes', if_exists=True)
    op.drop_table('roster_roles', if_exists=True)
    op.drop_table('roster', if_exists=True)
    op.drop_table('pathway_projects', if_exists=True)
    op.drop_table('achievements', if_exists=True)
    op.drop_table('Users', if_exists=True)
    op.drop_table('Meetings', if_exists=True)
    op.drop_table('Media', if_exists=True)
    op.drop_table('Session_Logs', if_exists=True)
    op.drop_table('Session_Types', if_exists=True)
    op.drop_table('roles', if_exists=True)
    op.drop_table('pathways', if_exists=True)
    op.drop_table('level_roles', if_exists=True)
