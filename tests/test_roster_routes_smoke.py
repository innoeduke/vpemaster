...
def test_roster_route_access(app, client, auth, staff_user, default_club, seeded_permissions, captured_templates):
    """Test that the roster route is accessible to authorized users."""
    with app.app_context():
        auth.login(username='staff', password='password', club_id=default_club.id)
...
def test_roster_trend_route_access(app, client, auth, staff_user, default_club, seeded_permissions, captured_templates):
...
def test_roster_api_get_entry_not_found(app, client, auth, staff_user, default_club, seeded_permissions):
